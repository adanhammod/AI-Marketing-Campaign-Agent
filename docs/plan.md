...
- **Week 2:** wire real HyperFrames/Image Generator MCP implementations behind the provider interface as access unblocks; the moment either produces one real success, bank it immediately as the canonical fallback asset. Minimal Prometheus `/metrics` on the worker's new health-probe endpoint, targeting the three approved dashboards. Terraform in dependency order: `s3`/`dynamodb`/`sqs`+DLQ/scoped `iam` first, then the kubeadm cluster (formalizing the Week 1 spike) with `dev`/`prod` namespaces, then Marketing MCP's own deployment/Service/container. Pin the Image Generator MCP's exact version into its (sidecar) image. Apply the security-group restriction and the no-production-TLS decision.
- **Week 3:** deploy to the cluster; PR quality gates, then GitHub Actions deploys to `dev`, validates, and promotes the same image to `prod` (self-approved, solo project). Add rate limiting on `POST /campaigns`/`POST /retry`. Rehearse the failure path deliberately — simulate a provider outage mid-run and confirm `fallback_asset=true` disclosure surfaces correctly. Reserve the last 2-3 days as slip buffer.
