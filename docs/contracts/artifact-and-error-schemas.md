...
`VALIDATION_ERROR`, `NOT_FOUND`, `STATE_CONFLICT`, `IDEMPOTENCY_CONFLICT`, `UNSUPPORTED_MESSAGE_SCHEMA`, `LEASE_CONFLICT`, `RETRY_EXHAUSTED`, `DELIVERY_RETRY_EXHAUSTED`, `CANCELLED_BY_USER`, `BEDROCK_UNAVAILABLE`, `IMAGE_PROVIDER_UNAVAILABLE`, `VOICE_PROVIDER_UNAVAILABLE`, `VIDEO_PROVIDER_UNAVAILABLE`, `PROVIDER_POLICY_REJECTION`, `PROVIDER_THROTTLED`, `PROVIDER_TIMEOUT`, `INVALID_PROVIDER_OUTPUT`, `ARTIFACT_VALIDATION_FAILED`, `STORAGE_UNAVAILABLE`, `QUEUE_UNAVAILABLE`, `PACKAGE_VALIDATION_FAILED`, and `INTERNAL_ERROR`.
