...
class _Cloudflare:
    """Fake standing in for whichever GenerativeImageClient is wired as
    primary (Cloudflare Workers AI / FLUX today; GeneratedImage's shape is
    provider-agnostic, reused from stability_client.py -- see that module's
    docstring)."""
    ...
    def __init__(self, fail_scenes: set[int] | None = None, error: WorkflowOperationError | None = None) -> None:
        ...
        self._error = error or WorkflowOperationError("PROVIDER_TIMEOUT", "generation timed out", retryable=True)
...
def _pipeline(
    cloudflare: _Cloudflare, store: _Store, fallback=None, model: str = "@cf/black-forest-labs/flux-1-schnell"
...
async def test_generative_success_produces_three_normalized_stored_image_artifacts():
    cloudflare, store = _Cloudflare(), _Store()
    result = await _pipeline(cloudflare, store).acquire(_version(), _false)
    ...
    assert cloudflare.scene_calls == [1, 2, 3]


@pytest.mark.asyncio
async def test_generative_artifacts_record_cloudflare_provider_and_model_in_generation_summary():
    cloudflare, store = _Cloudflare(), _Store()
    result = await _pipeline(cloudflare, store).acquire(_version(), _false)

    assert all(artifact.provider == "cloudflare-workers-ai" for artifact in result)
    assert all(artifact.attribution is None for artifact in result)
    for n in (1, 2, 3):
        assert store.metadata[n]["model"] == "@cf/black-forest-labs/flux-1-schnell"
        assert store.metadata[n]["aspect_ratio"] == "1:1"
        assert store.metadata[n]["provider"] == "cloudflare-workers-ai"
        assert "prompt_fingerprint" in store.metadata[n]


@pytest.mark.asyncio
async def test_identical_generative_inputs_reconcile_from_cache_without_calling_provider_twice():
    version = _version()
    cloudflare, store = _Cloudflare(), _Store()
    pipeline = _pipeline(cloudflare, store)
    ...
    assert second == first
    assert cloudflare.scene_calls == [1, 2, 3]  # no new calls
...
async def test_changed_scene_visual_prompt_invalidates_only_that_scenes_generative_cache():
    version = _version()
    store = _Store()
    await _pipeline(_Cloudflare(), store).acquire(version, _false)
    ...
    cloudflare2 = _Cloudflare()
    await _pipeline(cloudflare2, store).acquire(modified, _false)

    assert cloudflare2.scene_calls == [2]


@pytest.mark.asyncio
async def test_changed_model_invalidates_generative_cache_for_all_scenes():
    version = _version()
    store = _Store()
    await _pipeline(_Cloudflare(), store, model="@cf/black-forest-labs/flux-1-schnell").acquire(version, _false)

    cloudflare2 = _Cloudflare()
    await _pipeline(cloudflare2, store, model="@cf/black-forest-labs/flux-1-schnell-v2").acquire(version, _false)

    assert cloudflare2.scene_calls == [1, 2, 3]


@pytest.mark.asyncio
async def test_provider_timeout_falls_back_to_stock_pipeline_and_returns_pexels_artifacts():
    store = _Store()
    ...
    result = await _pipeline(cloudflare, store, fallback).acquire(_version(), _false)
...
@pytest.mark.asyncio
async def test_provider_unavailable_falls_back_to_stock_pipeline():
    store = _Store()
    cloudflare = _Cloudflare(
...
@pytest.mark.asyncio
async def test_provider_throttled_and_insufficient_credits_fall_back_to_stock_pipeline():
    for code, retryable in (("PROVIDER_THROTTLED", True), ("PROVIDER_THROTTLED", False)):
        store = _Store()
        cloudflare = _Cloudflare(fail_scenes={3}, error=WorkflowOperationError(code, "throttled", retryable=retryable))
        fallback = _FakeStockFallback(store)
...
@pytest.mark.asyncio
async def test_invalid_provider_output_falls_back_to_stock_pipeline():
    # Covers both a malformed image response AND a generic Cloudflare 400
    # (rejected/invalid prompt) -- both map to INVALID_PROVIDER_OUTPUT and
    # are fallback-approved (confirmed behavior).
    store = _Store()
    cloudflare = _Cloudflare(
...
async def test_when_fallback_pexels_also_fails_the_error_code_propagates_unmasked():
    store = _Store()
    cloudflare = _Cloudflare(fail_scenes={1})
    pipeline = _pipeline(cloudflare, store, _FailingFallback())
...
async def test_storage_unavailable_during_generative_put_is_not_treated_as_fallback_eligible():
    ...
    store = _FailingStore()
    fallback = _FakeStockFallback(store)
    pipeline = _pipeline(_Cloudflare(), store, fallback)
...
async def test_policy_rejection_fails_the_job_without_falling_back_to_pexels():
    ...
    fallback = _FakeStockFallback(store)
    pipeline = _pipeline(cloudflare, store, fallback)
...
async def test_partial_generative_persistence_never_produces_a_mixed_final_artifact_set():
    ...
    # 1. Seed scene 1 with a valid, reconcilable Cloudflare artifact, as if a
    ...
    fingerprint = generative_scene_fingerprint(
        version, scene_one, prompt, model="@cf/black-forest-labs/flux-1-schnell", aspect_ratio="1:1", output_format="jpeg"
    )
    image = ImageProcessor().normalize(_jpeg())
    store.put(
        version,
    ...
    # pre-existing, still-valid Cloudflare artifact.
    cloudflare = _Cloudflare(fail_scenes={2})
    fallback = _FakeStockFallback(store)
    result = await _pipeline(cloudflare, store, fallback).acquire(version, _false)

    assert {artifact.provider for artifact in result} == {"pexels"}
    assert len(result) == 3
    assert 1 not in cloudflare.scene_calls  # scene 1's cached artifact was never touched here
    ...
    # 5. Retry: the primary provider now succeeds for all 3 scenes. Scene 1
    # must NOT be served from its old, now-overwritten cache -- a fresh
    # generate() call is required for it too, and the final result must be
    # all-Cloudflare.
    cloudflare_retry = _Cloudflare()
    retry_result = await _pipeline(cloudflare_retry, store, fallback).acquire(version, _false)
    assert cloudflare_retry.scene_calls == [1, 2, 3]
    assert {artifact.provider for artifact in retry_result} == {"cloudflare-workers-ai"}

    # 6. Simulate scene 2's cache being invalidated again (e.g. a later
    # revision touching only that scene) and the primary provider failing on
    # the retry -- even though scenes 1 and 3 are still validly cached at
    # this point, the result must once more be all-Pexels, never a mix of
    # cached-Cloudflare + fresh-Pexels artifacts.
    del store.saved[2]
    del store.metadata[2]
    cloudflare_retry_fail = _Cloudflare(fail_scenes={2})
    second_fallback_result = await _pipeline(cloudflare_retry_fail, store, fallback).acquire(version, _false)
