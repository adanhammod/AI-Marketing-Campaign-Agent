...
def audio_plan_for(style: VideoStyle) -> AudioPlan:
    if style is VideoStyle.VOICEOVER_AD:
        return AudioPlan(voiceover_required=True, music_required=False, sfx_enabled=False)
    if style is VideoStyle.CINEMATIC_TEXT_AD:
        return AudioPlan(voiceover_required=False, music_required=True, sfx_enabled=True)
