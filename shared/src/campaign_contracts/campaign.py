...
from .artifacts import AudioArtifactReference, FinalPackageArtifactReference, ImageArtifactReference, VideoArtifactReference, PublicArtifactReference
from .enums import AssetRole, AudioCueType, CameraMotion, CampaignStatus, RevisionTarget, ShotRole, TransitionType, VideoStyle, WorkflowStep
...
class CampaignVersion(UTCModel):
    ...
    def version_parent(self):
        ...
        return self
    @model_validator(mode="after")
    def creative_plan_duration_within_bounds(self):
        if self.creative_video_plan is not None:
            total=self.creative_video_plan.total_duration_seconds
            if not (self.constraints.min_duration_seconds<=total<=self.constraints.max_duration_seconds): raise ValueError("creative_video_plan total_duration_seconds must be within campaign duration constraints")
        return self
...
def assert_version_immutable(before:CampaignVersion,after:CampaignVersion)->None:
    ...
    frozen={"brief","constraints","strategy","campaign_copy","storyboard","creative_video_plan","image_prompts","image_artifacts","voice_artifact","video_artifact","review_package"}
